...
class FetchEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        # TODO added intial_object_qvel here
    ...
    def _get_obs(self):
        ...
        if self.has_object:
            ...
            # TODO changed from relative to absolute
    ...
    def _reset_sim(self):
        ...
        # Removed so that fixed position picked up from provided values
        # if self.has_object:
    ...
    def _env_setup(self, initial_qpos, obj_init):
        ...
        for _ in range(10):
            self.sim.step() # Because end effector takes time to get into position

        ######### TODO Changed from here #########
        object_qvel = self.sim.data.get_joint_qvel("object0:joint")
        assert object_qvel.shape == (6,)
        obj_init = np.array(obj_init, dtype=np.float32)
        assert obj_init.shape == (2,)
        object_qvel[:2] = obj_init

        # Stopping rotations
        # object_qvel[3:] = np.zeros_like(object_qvel[3:])

        self.sim.data.set_joint_qvel("object0:joint", object_qvel)
        self.sim.forward()

        ######### TODO Changed till here #########
